...
function drawMolecule(index, lcmFile) {
	...
	FindMoleculeNamespace.drawMoleculeInCanvas(ctx, lcmFile, 150, 125);
}
...
function callStructureSearch() {
	...
	if (!structure) return;
	var canvases = $("#moleculeCanvases");
	var createD = $("<div>");
	createD.attr("class", "w-100 py-2 text-center rounded-sm border text-muted");
	canvases.html(""); createD.html("搜索中...").appendTo(canvases);
	$.ajax({
		type: "POST",
		headers: {
			'Accept': "*",
		...
		if (fmResults) {
			...
			if (fmResults.list.length === 0) createD.html("查无结果").appendTo(canvases);
		} else {
			createD.html("查无结果").appendTo(canvases);
		};
	}).fail(function (error) {
		console.error(error);
		canvases.html("");
		// var canvases = 
		// var createD = $("<div>");
		createD.html("查询失败").appendTo(canvases);
	});
}
